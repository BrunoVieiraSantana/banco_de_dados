...
Trânsito""", image=self.logo_image, compound="left", font=customtkinter.CTkFont(size=15, weight="bold"))
        ...
        self.home_frame_login_button = customtkinter.CTkButton(self.login_frame, text="Esqueceu sua senha?",command=self.forgot_pass, width=200, fg_color="transparent", text_color=("gray10", "gray90"))
        self.home_frame_login_button.grid(row=4, pady=5)
        self.home_frame_login_button = customtkinter.CTkButton(self.login_frame, text="Criar conta",command=self.new_user, width=200, fg_color="transparent", text_color=("gray10", "gray90"))
...
que deseja remover""",
        ...
        self.seventh_frame_entry_data = customtkinter.CTkEntry(self.seventh_frame , placeholder_text="Data de Nascimetno  (dd/mm/aa)",width=250)
        self.seventh_frame_entry_data.grid(row=5, column=0,  pady=5, sticky='n')
        self.seventh_frame_entry_email = customtkinter.CTkEntry(self.seventh_frame , placeholder_text="E-mail",width=250)
        self.seventh_frame_entry_email.grid(row=6, column=0,  pady=(5,5), sticky='n')
        self.seventh_frame_button_buscar = customtkinter.CTkButton(self.seventh_frame , fg_color="transparent" ,command=inserirLogin, border_width=2,text="Cadastrar", text_color=("gray10", "#DCE4EE"))
        self.seventh_frame_button_buscar.grid(row=7, column=0,  pady=(5,5), sticky='n')
        ...
        self.eighth_frame_button_buscar = customtkinter.CTkButton(self.eighth_frame , fg_color="transparent" , border_width=2,text="buscar", text_color=("gray10", "#DCE4EE"))
...
#Botão para deletar motorista
    ...
    def button_add_motorista(self):
        ...
        if len(name) == 0 or len(cpf) == 0 or len(cnh) == 0 or len(cnh) >= 12:
            show_error_empty()
...
# Login fuctions
    ...
    def button_login_user(self):
        ...
        if len(login) == 0 or len(senha) == 0: 
            ...
            login_db = 'admin'
            admin_db = 'admin'
            if login == login_db and senha == admin_db:
